...
All project activity is subject to the repository's [Code of
Conduct](./CODE_OF_CONDUCT.md). Repository maintainers will document any
additional contribution agreement before requiring it.

Orion Studio is derived from the open-source Zed editor. Preserve upstream
copyright notices, contributor attribution, and the applicable
[GPL](./LICENSE-GPL) and [Apache](./LICENSE-APACHE) license terms. See
[Upstream Attribution and Compatibility](./docs/src/orion-studio-upstream-and-compatibility.md)
before renaming historical crates, extension identifiers, or compatibility
paths.

## Contribution ideas

Orion Studio is a large project with a number of priorities. We spend most of
...
- Issues suitable for [first-time contributors](https://github.com/orion-agents/orion-studio/issues?q=is%3Aissue%20state%3Aopen%20label%3A%22.contrib%2Fgood%20first%20issue%22) and [returning contributors](https://github.com/orion-agents/orion-studio/issues?q=state%3Aopen%20label%3A%22.contrib%2Fgood%20non-first%20issue%22)
...
- Make sure the change is **desired**: we're always happy to accept bugfixes,
  ...
  - This especially applies to any changes proposed to the Orion Studio Extension API.
- Include a clear description of **what you're solving**, and why it's important.
- Include **tests**. For UI changes, consider updating visual regression tests (see [Building Orion Studio for macOS](./docs/src/development/macos.md#visual-regression-tests)).
...
- Anything that can be provided by an extension. For example a new language, or theme. For adding themes or support for a new language to Orion Studio, check out our [docs on developing extensions](./docs/src/extensions/developing-extensions.md).
- Changes to the Orion Studio Extension API submitted without prior discussion involving Orion Studio maintainers. The published Rust crate and WIT ABI retain `zed_extension_api` and `zed:extension` compatibility identifiers.
- New file icons. Orion Studio's default icon theme consists of icons that are hand-designed to fit together in a cohesive manner, please don't submit PRs with off-the-shelf SVGs.
...
- Does it match Orion Studio’s design language (spacing, typography, icons)?
...
- Are terminology, labels, and tone consistent with the rest of Orion Studio?
...
- Do we avoid internal Orion Studio jargon that only insiders would know?
...
- Are error messages actionable and consistent with Orion Studio’s voice?
...
## Bird's-eye view of Orion Studio

We suggest you keep the [Orion Studio glossary](docs/src/development/glossary.md) at your side when starting out. It lists and explains some of the structures and terms you will see throughout the codebase.

Orion Studio is made up of several smaller crates - let's go over those you're most likely to interact with:

- [`gpui`](./crates/gpui/) is a GPU-accelerated UI framework which provides all of the building blocks for Orion Studio. **We recommend familiarizing yourself with the root level GPUI documentation.**
- [`editor`](./crates/editor/) contains the core `Editor` type that drives both the code editor and all various input fields within Orion Studio. It also handles a display layer for LSP features such as Inlay Hints or code completions.
- [`project`](./crates/project/) manages files and navigation within the filetree. It is also Orion Studio's side of communication with LSP.
...
- [`ui`](./crates/ui/) is a collection of UI components and common patterns used throughout Orion Studio.
- [`cli`](./crates/cli/) is the CLI crate which invokes the Orion Studio binary.
- [`zed`](./crates/zed/) is the historical crate directory where the application
  comes together. Its package and main binary are named `orion-studio`; the
  directory name remains for repository compatibility.

## Packaging Orion Studio

Check our [notes for packaging Orion Studio](./docs/src/development/linux.md#notes-for-packaging-orion-studio).
