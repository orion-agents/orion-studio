...
> Orion Studio is implemented in Rust with GPU-accelerated rendering. Publish input-latency or frame-rate numbers only when a current benchmark supports them.
...
> Orion Studio keeps common workflows close together: jumping between files, searching across projects, and editing with multiple cursors.
...
> We're thrilled to announce Orion Studio <version>! After years of hard work, we can't wait for you to experience what we've built. You're going to love it!

**After (Score: 5/5)**

> Orion Studio <version> is available today. This release includes GPU text rendering, multi-buffer editing, and native collaboration. Download it at orion.dev.
...
> Orion Studio combines editing, debugging, collaboration, and AI assistance in one application. Related workflows share one native workspace.
...
> Orion Studio can show AI completions inline as you type. Accept with Tab or continue typing to reject a suggestion. The configured provider determines where the model runs.
...
> Orion Studio <version>: Multi-buffer editing is here. Edit across files in a single view. AI completions now stream inline. Full changelog at orion.dev/releases.
...
> Orion Studio includes a built-in terminal. Open it with `ctrl+\``. Terminals run in splits alongside editor panes and share the same working directory as the project.
...
> Developer tools should be responsive, understandable, and collaborative. Orion Studio is open source, so you can inspect its implementation and extend it.
...
```
Orion Studio is [FACT: implemented in Rust] with [FACT: GPU-accelerated rendering].
It is [FACT: derived from the upstream Zed project].
```
...
| Version numbers    | "Orion Studio <version>"                    |
| Keyboard shortcuts | "cmd+shift+c", "ctrl+\`"                   |
| URLs               | "orion.dev/releases"                       |
| Attribution        | "derived from the upstream Zed project"    |
...
> Orion Studio uses a Rust codebase and GPU-accelerated rendering. It retains attribution to the upstream Zed project from which it is derived.

**Verification:**

- ✅ "Rust" preserved
- ✅ "GPU-accelerated rendering" preserved
- ✅ upstream Zed attribution preserved
- **Pass**
