...
Search **only** issues in the current Orion Studio repository, `orion-agents/orion-studio` (prefer `gh issue list` / `gh issue view` / GraphQL if available), by:
...
Check both open and recently closed issues in `orion-agents/orion-studio`.

If the analysis identifies inherited upstream behavior, you may run a separate advisory search in the upstream Zed repository at `https://github.com/zed-industries/zed`. Label those results as upstream references, never use Orion Studio closing keywords for them, and do not mix them into the primary Orion Studio confidence groups.
...
## High Confidence
- [#12345](https://github.com/orion-agents/orion-studio/issues/12345) — <title>
...
- Keep the primary shortlist limited to `orion-agents/orion-studio`; put any explicitly requested upstream Zed references in a separate advisory section.
