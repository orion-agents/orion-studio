...
/// (Anthropic's `error.type` strings, OpenRouter's numeric codes, Orion cloud's
...
/// category so the rest of Orion Studio only needs to understand one vocabulary.
///
...
pub enum LanguageModelCompletionError {
    ...
    /// opposed to a transport or plumbing failure on Orion Studio's side.
    ///
