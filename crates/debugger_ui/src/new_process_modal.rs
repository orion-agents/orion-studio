...
use task::{DebugScenario, OrionDebugConfig, RevealTarget, SharedTaskContext, VariableName};
use ui::{
...
impl NewProcessModal {
    ...
    fn debug_scenario(&self, debugger: &str, cx: &App) -> Task<Option<DebugScenario>> {
        ...
        let session_scenario = OrionDebugConfig {
        ...
        cx.spawn(async move |_| {
            adapter?
                .config_from_orion_format(session_scenario)
                .await
                .ok()
        })
    }
...
impl ConfigureMode {
    pub(super) fn new(window: &mut Window, cx: &mut App) -> Entity<Self> {
        let program = cx.new(|cx| {
            InputField::new(window, cx, "ENV=OrionStudio ~/bin/program --option")
                .label("Program")
                .tab_stop(true)
                .tab_index(1)
        });
        ...
        let cwd = cx.new(|cx| {
            InputField::new(window, cx, "Ex: $ORION_STUDIO_WORKTREE_ROOT")
...
pub(super) struct AttachMode {
    pub(super) definition: OrionDebugConfig,
...
impl AttachMode {
    pub(super) fn new(
        ...
        let definition = OrionDebugConfig {
