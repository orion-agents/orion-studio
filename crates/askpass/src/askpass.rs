...
fn generate_gpg_wrapper_script(
    ...
    // without Orion Studio getting in the way:
    ...
    //   3. Loopback mode with the passphrase from Orion Studio's askpass modal
    //      (fd 3), for setups where gpg cannot prompt at all.
...
# Stage 3: gpg cannot obtain the passphrase on its own. Ask Orion Studio for
# it, then hand it to gpg on fd 3 using loopback mode so no pinentry/terminal
# is required.
