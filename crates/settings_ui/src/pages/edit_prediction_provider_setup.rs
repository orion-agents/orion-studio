...
pub(crate) fn render_edit_prediction_setup_page(
    ...
    let providers = [
        ...
        Some(render_orion_provider(settings_window, window, cx).into_any_element()),
                ...
                .child(
                    v_flex()
                        ...
                        .when_some(env_var_name, |this, env_var_name| {
                            this.child({
                                let label = format!(
                                    "Or set the {} env var and restart Orion Studio.",
                                    env_var_name.as_ref()
                                );
...
fn orion_settings() -> Box<[SettingsPageItem]> {
    Box::new([SettingsPageItem::SettingItem(SettingItem {
        title: "Prediction Debounce",
        description: "Delay in milliseconds before automatically requesting a prediction after typing stops. Set to 0 to request predictions immediately.",
        field: Box::new(SettingField {
...
fn render_orion_provider(
    settings_window: &SettingsWindow,
    window: &mut Window,
    cx: &mut Context<SettingsWindow>,
) -> impl IntoElement {
    let orion_settings = orion_settings();
    let additional_fields = settings_window
        .render_sub_page_items_section(orion_settings.iter().enumerate(), true, window, cx)
        .into_any_element();
    ...
    v_flex()
        ...
        .gap_1p5()
        .child(
            SettingsSectionHeader::new("Orion AI Predictions")
                .icon(IconName::OrionPredict)
                .no_padding(true),
        )
        .child(div().px_neg_8().child(additional_fields))
}
