...
use agent::{ORION_AGENT_ID, ThreadStore, canonicalize_native_agent_id, is_native_agent_id};
...
pub struct ArchivedGitWorktree {
    ...
    /// the worktree was in detached HEAD state, which isn't supported in Orion Studio, but
    /// could happen if the user made a detached one outside of Orion Studio.
...
impl ThreadMetadataDb {
    ...
    pub async fn save(&self, row: ThreadMetadata) -> anyhow::Result<()> {
        ...
        let agent_id = if is_native_agent_id(row.agent_id.as_ref()) {
            None
        } else {
...
impl Column for ThreadMetadata {
    fn column(statement: &mut Statement, start_index: i32) -> anyhow::Result<(Self, i32)> {
        ...
        let agent_id = agent_id
            ...
            .map(canonicalize_native_agent_id)
            .unwrap_or(ORION_AGENT_ID.clone());
...
mod tests {
    ...
    async fn test_migrate_thread_metadata_migrates_only_missing_threads(cx: &mut TestAppContext) {
        ...
        assert!(
            list.iter()
                .all(|metadata| metadata.agent_id.as_ref() == agent::ORION_AGENT_ID.as_ref())
        );
