...
impl PythonDebugAdapter {
    ...
    async fn request_args(
        ...
        if let Ok(console) = configuration.dot_get_mut("console") {
            // Use the built-in Orion Studio terminal if the user did not explicitly configure the console.
            if console.is_null() {
        ...
        let result = self.base_venv_path
            .get_or_init(|| async {
                ...
                let output = util::command::new_command(&base_python)
                    .args(["-m", "venv", "orion_studio_base_venv"])
                    .current_dir(
                        &debug_adapter_path,
                    )
                    .spawn()
...
impl DebugAdapter for PythonDebugAdapter {
    ...
    async fn config_from_orion_format(
        &self,
        orion_scenario: OrionDebugConfig,
    ) -> Result<DebugScenario> {
        let mut args = json!({
            "request": match orion_scenario.request {
        ...
        match &orion_scenario.request {
            ...
            DebugRequest::Launch(launch) => {
                ...
                if let Some(stop_on_entry) = orion_scenario.stop_on_entry {
        ...
        Ok(DebugScenario {
            adapter: orion_scenario.adapter,
            label: orion_scenario.label,
            config: args,
            build: None,
            tcp_connection: None,
        })
            ...
            "properties": {
                ...
                "pathMappings": {
                    ...
                    "items": {
                        ...
                        "properties": {
                            "localRoot": {
                                "default": "${ORION_STUDIO_WORKTREE_ROOT}",
                ...
                },
                {
                    ...
                    "then": {
                        "properties": {
                            ...
                            "envFile": {
                                "default": "${ORION_STUDIO_WORKTREE_ROOT}/.env",
    ...
    async fn get_binary(
        ...
        let base_paths = ["cwd", "program", "module"]
            ...
            .chain(
                ...
                // (Which should always be the case because Orion Studio defaults cwd to the worktree root)
