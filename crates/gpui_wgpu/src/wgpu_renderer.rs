...
impl RenderingParameters {
    fn new(adapter: &wgpu::Adapter, surface_format: wgpu::TextureFormat) -> Self {
        ...
        let gamma = env::var("ORION_STUDIO_FONTS_GAMMA")
            .or_else(|error| {
        ...
        let grayscale_enhanced_contrast =
            env::var("ORION_STUDIO_FONTS_GRAYSCALE_ENHANCED_CONTRAST")
                .or_else(|error| {
                    if matches!(error, env::VarError::NotPresent) {
        ...
        let subpixel_enhanced_contrast = env::var("ORION_STUDIO_FONTS_SUBPIXEL_ENHANCED_CONTRAST")
            .or_else(|error| {
                if matches!(error, env::VarError::NotPresent) {
