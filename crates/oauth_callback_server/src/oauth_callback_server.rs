...
//!
//! Used by Orion Studio's OAuth-based sign-in flows (e.g. MCP servers, ChatGPT
...
/// layout styled to match Orion Studio's dark theme. The `title` is rendered as a
...
<title>{title} — Orion Studio</title>
<style>
...
<div class="card">
  ...
  <h1>{title}</h1>
  <p>{message}</p>
  <div class="brand">Orion Studio</div>
</div>
</body>
</html>"#,
        title = title,
...
mod tests {
    ...
    #[test]
    fn callback_page_uses_orion_studio_brand() {
        let page = oauth_callback_page("Authorization Successful", "Return to the app.", false);

        assert!(page.contains("<title>Authorization Successful — Orion Studio</title>"));
        assert!(page.contains("<div class=\"brand\">Orion Studio</div>"));
                ...
                let (status_code, body) = match &result {
                    Ok(_) => (
                        ...
                        oauth_callback_page(
                            ...
                            "You can close this tab and return to Orion Studio.",
                            false,
                        ),
                    ),
                    Err(err) => {
                        ...
                        (
                            ...
                            oauth_callback_page(
                                ...
                                "Something went wrong. Please try again from Orion Studio.",
                                true,
                            ),
                        )
                    }
