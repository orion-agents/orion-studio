...
#[gpui::test]
async fn test_edit_prediction_jump_disabled_for_non_orion_providers(cx: &mut gpui::TestAppContext) {
    init_test(cx, |_| {});

    let mut cx = EditorTestContext::new(cx).await;
    let provider = cx.new(|_| FakeNonOrionEditPredictionDelegate::default());
    assign_editor_completion_provider_non_orion(provider.clone(), &mut cx);
    ...
    // For non-Orion providers, there should be no move completion (jump functionality disabled)
    cx.editor(|editor, _, _| {
        if let Some(completion_state) = &editor.active_edit_prediction {
            ...
            match &completion_state.completion {
                EditPrediction::Edit { .. } => {
                    // This is expected for non-Orion providers
                }
                EditPrediction::MoveWithin { .. } | EditPrediction::MoveOutside { .. } => {
                    panic!(
                        "Non-Orion providers should not show Move predictions (jump functionality)"
                    );
                }
            }
        }
...
fn propose_edits_non_orion<T: ToOffset>(
    provider: &Entity<FakeNonOrionEditPredictionDelegate>,
    edits: Vec<(Range<T>, &str)>,
    cx: &mut EditorTestContext,
) {
    let snapshot = cx.buffer_snapshot();
...
fn assign_editor_completion_provider_non_orion(
    provider: Entity<FakeNonOrionEditPredictionDelegate>,
    cx: &mut EditorTestContext,
) {
    cx.update_editor(|editor, window, cx| {
...
impl EditPredictionDelegate for FakeEditPredictionDelegate {
    ...
    fn icons(&self, _cx: &gpui::App) -> EditPredictionIconSet {
        EditPredictionIconSet::new(IconName::OrionPredict)
    }
...
#[derive(Default, Clone)]
pub struct FakeNonOrionEditPredictionDelegate {
    pub completion: Option<edit_prediction_types::EditPrediction>,
}

impl FakeNonOrionEditPredictionDelegate {
    pub fn set_edit_prediction(
        &mut self,
        completion: Option<edit_prediction_types::EditPrediction>,
    ) {
...
impl EditPredictionDelegate for FakeNonOrionEditPredictionDelegate {
    fn name() -> &'static str {
        "fake-non-orion-provider"
    }
