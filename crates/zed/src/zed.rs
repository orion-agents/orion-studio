...
fn initialize_panels(window: &mut Window, cx: &mut Context<Workspace>) -> Task<anyhow::Result<()>> {
    ...
    cx.spawn_in(window, async move |workspace_handle, cx| {
        ...
        let task_environment_panel =
            agent_ui::TaskEnvironmentPanel::load(workspace_handle.clone(), cx.clone());
        ...
        futures::join!(
            ...
            add_panel_when_ready(task_environment_panel, workspace_handle.clone(), cx.clone()),
            channels_panel,
...
/// Starts watching `~/.config/orion-studio/AGENTS.md` (or the platform equivalent) and
