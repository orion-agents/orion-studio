...
use language_model::{LanguageModelRegistry, ORION_CLOUD_PROVIDER_ID};
use ui::prelude::*;

use crate::{AgentPanelOnboardingCard, ApiKeysWithoutProviders, OrionAiOnboarding};
...
impl AgentPanelOnboarding {
    pub fn new(
        user_store: Entity<UserStore>,
        client: Arc<Client>,
        continue_with_orion_ai: impl Fn(&mut Window, &mut App) + 'static,
        cx: &mut Context<Self>,
    ) -> Self {
        cx.subscribe(
        ...
        Self {
            ...
            continue_with_orion_ai: Arc::new(continue_with_orion_ai),
        }
    }
    ...
    fn has_configured_providers(cx: &App) -> bool {
        LanguageModelRegistry::read_global(cx)
            ...
            .iter()
            .any(|provider| {
                provider.is_authenticated(cx) && provider.id() != ORION_CLOUD_PROVIDER_ID
            })
    }
}
...
impl Render for AgentPanelOnboarding {
    fn render(&mut self, _window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let onboarding = OrionAiOnboarding::new(
            self.client.clone(),
            &self.user_store,
        ...
        .with_dismiss({
            let callback = self.continue_with_orion_ai.clone();
