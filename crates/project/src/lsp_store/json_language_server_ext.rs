...
/// In our case, we provide the URIs for actions on server startup under the
/// `orion://schemas/action/{normalize_action_name}` scheme while accepting the
/// former scheme for compatibility.
