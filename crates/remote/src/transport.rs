...
fn parse_platform(output: &str) -> Result<RemotePlatform> {
    ...
    let os = match os {
        ...
        _ => anyhow::bail!(
            "Prebuilt remote servers are not yet available for {os:?}. See https://github.com/orion-agents/orion-studio/blob/main/docs/src/remote-development.md"
        ),
    };
        ...
        || arch.starts_with("aarch64")
    {
        ...
        anyhow::bail!(
            "Prebuilt remote servers are not yet available for {arch:?}. See https://github.com/orion-agents/orion-studio/blob/main/docs/src/remote-development.md"
        )
    };
