    ...
    async fn platform_windows(&self, shell: ShellKind) -> Result<RemotePlatform> {
        ...
        Ok(RemotePlatform {
            ...
            arch: match output.trim() {
                ...
                arch => anyhow::bail!(
                    "Prebuilt remote servers are not yet available for windows-{arch}. See https://github.com/orion-agents/orion-studio/blob/main/docs/src/remote-development.md"
                ),
            },
        })
    }
