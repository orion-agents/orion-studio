...
```markdown
![Alt text description](https://orion.dev/img/path/to/image.webp)
```
...
- **Practical over promotional**: Focus on what users can do, not on selling Orion Studio. Avoid marketing language like "powerful," "revolutionary," or "best-in-class."
- **Honest about limitations**: When Orion Studio lacks a feature or doesn't match another tool's depth, say so directly. Pair limitations with workarounds or alternative workflows.
...
- **Present tense**: "Orion Studio opens the file" not "Orion Studio will open the file."
...
```
Orion Studio provides a powerful and seamless formatting experience. Simply navigate to the settings and you'll find the format_on_save option which enables Orion Studio's incredible auto-formatting capabilities.
```

### Good: Honest about limitations

```
Orion Studio doesn't index your project like IntelliJ does. You open a folder and start working immediately—no waiting. The trade-off: cross-project analysis relies on language servers, which may not go as deep.
...
```
While some users might miss indexing, Orion Studio's approach is actually better because it's faster.
```
...
## Orion Studio-Specific Conventions

### Recognized Rules Files

When documenting rules/instructions for AI, note that Orion Studio recognizes these files (in priority order):
...
- Linux: `~/.config/orion-studio/settings.json`
- Windows: `%AppData%\Orion Studio\settings.json`

### Keymap File Locations

- macOS: `~/.config/orion-studio/keymap.json`
- Linux: `~/.config/orion-studio/keymap.json`
