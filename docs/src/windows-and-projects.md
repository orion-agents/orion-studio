...
description: "How Orion Studio handles multiple projects in windows, including the threads sidebar and options for opening in new windows."
---

# Windows & Projects

Orion Studio lets you work on multiple projects in a single window. Projects appear in the threads sidebar on the left, and you can switch between them while keeping your context intact.
...
By default, when you open a folder in Orion Studio, it opens as a new project in your current window's threads sidebar rather than creating a new window. This keeps related work together and preserves your agent threads and layout.

| Action                      | Result                                    |
| --------------------------- | ----------------------------------------- |
| File > Open                 | Opens in current window (threads sidebar) |
| File > Open Recent          | Opens in current window (threads sidebar) |
| Drag folder to Orion Studio | Opens in current window (threads sidebar) |
| `orion ~/project`           | Opens in current window (threads sidebar) |
...
```sh
orion -n ~/projects/other-project
```
...
- [VS Code Migration](./migrate/vs-code.md): How Orion Studio's project model differs from VS Code
