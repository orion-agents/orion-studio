---
title: Use a Gateway - Orion Studio
description: Configure OpenRouter, Vercel AI Gateway, Amazon Bedrock, and other gateway or cloud model platforms in Orion Studio.
---
...
| Gateway                   | Orion Studio AI features | External Agents | Terminal Threads | Notes                                        |
| ------------------------- | ------------------------ | --------------- | ---------------- | -------------------------------------------- |
...
Use OpenRouter when you want to route Orion Studio AI features through OpenRouter.
...
Orion Studio also reads `OPENROUTER_API_KEY` from the local Orion Studio process environment.
...
Use Vercel AI Gateway when you want to route Orion Studio AI features through Vercel.
...
Orion Studio also reads `VERCEL_AI_GATEWAY_API_KEY` from the local Orion Studio process environment.
...
Bedrock supports the following isolated AWS environment variables so Orion
Studio does not override or consume your normal AWS credentials:

- `ORION_STUDIO_ACCESS_KEY_ID`
- `ORION_STUDIO_SECRET_ACCESS_KEY`
- `ORION_STUDIO_SESSION_TOKEN`
- `ORION_STUDIO_AWS_PROFILE`
- `ORION_STUDIO_AWS_REGION`
- `ORION_STUDIO_AWS_ENDPOINT`
- `ORION_STUDIO_BEDROCK_BEARER_TOKEN`

Legacy `ZED_*` names remain accepted only when the corresponding canonical
variable is absent.
...
Orion Studio uses [Cross-Region inference](https://docs.aws.amazon.com/bedrock/latest/userguide/cross-region-inference.html) for Bedrock on a best-effort basis.

By default, Orion Studio uses regional inference profiles. To opt into global profiles, add `allow_global`:
...
`protocol` selects which OpenAI-compatible API the model is called through, and must be either `chat_completions` or `responses`. Set `supports_thinking` to `true` for custom Mantle models that accept OpenAI reasoning effort parameters; Orion Studio will then expose `low`, `medium`, `high`, and `xhigh` in the thinking effort picker, while disabling thinking sends `none`.
