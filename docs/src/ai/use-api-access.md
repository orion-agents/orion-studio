---
title: Use API Access - Orion Studio
description: Configure provider API access, API keys, API credits, usage billing, and OpenAI-compatible endpoints for Orion Studio AI features.
---
...
Orion Studio supports these first-class API providers for model-backed Orion Studio AI features:
...
Use API access for the Orion Studio Agent, Inline Assistant, Git commit generation,
thread summaries, and similar Orion Studio-owned AI features.
...
Most API-access providers can be configured on the **Settings → AI → LLM Providers** page with {#action agent::OpenSettings}. Keys saved through Orion Studio are stored in the system keychain, not in `settings.json`.

Orion Studio also reads provider-specific environment variables. Non-empty environment variables take precedence over keychain values. If a key comes from an environment variable, unset the variable and restart Orion Studio to stop using it.
...
You can attach extra HTTP headers to every request Orion Studio makes to supported HTTP-based providers. This is useful in corporate environments or for observability tooling.
...
Headers managed by Orion Studio for each provider, such as `Authorization`, `Content-Type`, `Accept`, and provider-specific authentication headers, are ignored with a warning if you try to override them.

## Remote Projects {#remote-projects}

Orion Studio LLM providers for Orion Studio AI features are initialized in the local Orion Studio app. In SSH, dev container, and other remote projects, API keys saved in Orion Studio are read from the local system keychain, and provider environment variables are read from the local Orion Studio process environment.
...
Orion Studio also reads `ANTHROPIC_API_KEY` from the local Orion Studio process environment.
...
Orion Studio also reads `OPENAI_API_KEY` from the local Orion Studio process environment.
...
Orion Studio reads `GEMINI_API_KEY`, falling back to `GOOGLE_AI_API_KEY`, from the local Orion Studio process environment.
...
Orion Studio also reads `MISTRAL_API_KEY` from the local Orion Studio process environment.
...
Use DeepSeek API access when you have paid API usage, top-ups, or an API key. In Orion Studio, DeepSeek is API access, not subscription sign-in.
...
Orion Studio also reads `DEEPSEEK_API_KEY` from the local Orion Studio process environment.
...
Orion Studio also reads `XAI_API_KEY` from the local Orion Studio process environment.
...
Orion Studio does not sign in to OpenCode with OAuth or detect your OpenCode subscription; it uses an OpenCode API key saved in the system keychain or `OPENCODE_API_KEY`.
...
Orion Studio also reads `OPENCODE_API_KEY` from the local Orion Studio process environment.
...
**Note:** Orion Studio only bundles configuration for long-term OpenCode Free models. Free models that are available for a limited time are not included in Orion Studio. To use those models, add a custom OpenCode model with configuration from [the OpenCode website](https://opencode.ai/docs/zen#pricing) and [models.dev](https://github.com/anomalyco/models.dev/tree/dev/providers/opencode/models).

#### Custom OpenCode Models {#opencode-custom-models}

The Orion Studio Agent comes preconfigured with OpenCode models. Add custom OpenCode models when you need newer models, limited-time Free models, or models with custom endpoints.
...
The optional `custom_headers` map adds extra headers to every request, which some providers require. Headers managed by Orion Studio (such as `X-Api-Key` and `Anthropic-Version`) cannot be overridden.
...
If a model only works with the Responses API, set `capabilities.chat_completions` to `false`. Orion Studio will use the Responses endpoint for that model.

For reasoning models (e.g. GPT-5), set `reasoning_effort` to the non-`none` effort level your endpoint supports. This enables thinking in the agent panel and tells Orion Studio which effort to send when thinking is enabled. The provider settings UI can configure this when adding an OpenAI-compatible provider. Orion Studio sends OpenAI-style `reasoning_effort` on chat-completions requests.
