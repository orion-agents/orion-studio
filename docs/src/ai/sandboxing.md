---
title: Sandboxing
description: Orion Studio Agent tool calls can run in an OS-level sandbox to restrict certain operations.
---

# Sandboxing

You can restrict what operations the [Orion Studio Agent](./zed-agent.md) can run in multiple ways. One way to restrict them is
...
Sandboxing applies only to Orion Studio Agent. It does not sandbox Orion Studio itself, language servers, extensions, tasks, your normal
...
Orion Studio Agent sandboxing currently applies to the `terminal` and `fetch` tools.
...
By default, sandboxed Orion Studio Agent tool actions have these restrictions:
...
have tested thoroughly, there may also be bugs in Orion Studio's implementation. These
...
Orion Studio agent can do. It has **no effect** on other parts of Orion Studio, including:
...
Orion Studio's default profile aims to strike a balance between security and convenience,
...
When the agent needs access outside the default sandbox, Orion Studio shows a sandbox approval prompt before the tool action runs.
...
| `allow_unsandboxed`  | Turn sandboxing off entirely for Orion Studio Agent terminal commands. The fetch tool will have no restrictions.  |
...
On macOS, Orion Studio uses Apple's Seatbelt sandbox through `sandbox-exec`.
...
When network access is approved on macOS, Orion Studio uses an HTTP/HTTPS proxy so access can be limited to approved hosts.
...
On Linux, Orion Studio uses [Bubblewrap][bubblewrap] (`bwrap`) for sandboxing.

Orion Studio only uses a non-setuid `bwrap` binary. Its sandbox is built entirely on unprivileged user namespaces, so a setuid-root
`bwrap` provides no extra functionality, and running one would mean executing root-privileged setup with arguments partly
derived from model-influenced input. If the only `bwrap` found on your `PATH` is setuid-root, Orion Studio refuses to run it;
...
When host-specific network access is approved on Linux, Orion Studio uses an HTTP/HTTPS proxy so access can be limited to approved
...
If Bubblewrap is unavailable or cannot create a sandbox in the current environment, Orion Studio may run the command without the OS
...
Orion Studio needs a runnable, non-setuid `bwrap` binary on your `$PATH`. Installing
...
being phased out for security concerns, and so Orion Studio's sandbox _explicitly rejects
...
On Windows, Orion Studio Agent sandboxing is supported only when the agent action runs inside WSL.
...
> current-project grant for projects stored on NTFS. Orion Studio will show a warning
...
Orion Studio uses the Linux Bubblewrap sandbox inside WSL because WSL provides the Linux process and filesystem primitives that
Bubblewrap needs. Native Windows processes do not currently have the same sandbox integration in Orion Studio, so a native Windows
command cannot be confined by Orion Studio Agent's OS sandbox in the same way.
...
If WSL is not installed, or if you choose to run a command without the sandbox, Orion Studio falls back to the standard terminal
...
Orion Studio's sandbox has been designed with the aim of being totally unbreakable, even
