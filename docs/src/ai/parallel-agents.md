---
title: Parallel Agents - Orion Studio
...
To restore a thread, open Thread History and click the thread you want to bring back. Orion Studio moves it back to the thread list and opens it in the Agent Panel. If the thread was running in a Git worktree that was removed, Orion Studio restores the worktree automatically.
...
If you have External Agents installed, Orion Studio will detect whether you have existing threads and invite you to import them into Orion Studio. Once you open Thread History, you'll find an import icon button in the Thread History toolbar that lets you import threads at any time. Clicking on it opens a modal where you can select the agents whose threads you want to import.
...
Each thread can use a different agent, so you can run Orion Studio's built-in agent in one thread and an [External Agent](./external-agents.md) like Claude Code or Codex in another.
...
| --------------------------------------------- | -------------------------------------------------------------------------------- |
| [Orion Studio Agent thread](./zed-agent.md)   | Uses Orion Studio Agent settings, profiles, tools, Skills, Instructions, and MCP |
...
The Threads Sidebar can hold multiple projects at once. Each project gets its own group with its own threads and conversation history. This mirrors how Orion Studio handles projects in general — see [Windows & Projects](../windows-and-projects.md) for more on how projects open and how to manage them.
...
To automate setup steps whenever a new worktree is created, use a [Task hook](../tasks.md#hooks). The `create_worktree` hook runs automatically after Orion Studio creates a linked worktree, with the retained task API variables `ZED_WORKTREE_ROOT` pointing at the new worktree and `ZED_MAIN_GIT_WORKTREE` pointing at the original repository.
...
- [Terminal Threads](./terminal-threads.md): Run agent CLIs and TUIs directly in Orion Studio
