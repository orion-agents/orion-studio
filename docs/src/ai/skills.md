---
title: Agent Skills - Orion Studio
description: Extend Orion Studio's AI agent with reusable, on-demand skill files for specialized tasks.
---
...
Orion Studio includes a built-in `create-skill` skill — invoke it with `/create-skill` and the agent walks you through the process.
...
Lastly, it's also possible to add a skill through importing it from an existing GitHub Markdown file. Open the command palette and look for the {#action agent::CreateSkillFromUrl} action. If your clipboard contains a supported GitHub `.md` URL, Orion Studio pre-fills and fetches it automatically.
...
Open the Settings Editor (`Cmd+,` on macOS, `Ctrl+,` on Linux/Windows) and navigate to **AI > Skills**, or go directly to [agent.skills](orion://settings/agent.skills).

The **User** tab shows your global skills, and each **Project** tab shows the skills for that project.

For each skill you can:

- **Copy Share Link** — copies an `orion://skill` link that embeds the skill, ready to send to someone else (see [Sharing Skills](#sharing-skills))
...
You can hand a skill to a teammate without hosting it anywhere. In the Skills settings page, click the **link** icon on a skill row to copy an `orion://skill?data=…` link to your clipboard. Builds may continue to accept `zed://skill` as a legacy compatibility route; new links should use `orion://`.
...
When someone opens that link (for example by pasting it into their browser or clicking it in a chat), Orion Studio opens the "Create Skill" page in the settings window, pre-filled with the shared skill.
...
When the agent invokes a skill you created or installed, Orion Studio prompts you to allow or deny it, using the same permission flow as other tools. Skills built into Orion Studio do not prompt. You can set per-skill defaults in [Tool Permissions](./tool-permissions.md) so you're not prompted for skills you always trust.
...
Orion Studio loads skills from two locations:
...
Orion Studio Skills apply to the Orion Studio Agent. External Agents and Terminal Threads may have their own native skills, prompts, or instruction systems. Configure those in the External Agent or CLI.
