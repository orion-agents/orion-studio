...
description: "Guide to using a debugger for Orion Studio development."
---

# Using a debugger

> This page is not about [configuring Orion Studio's debugger](../debugger.md).
> It covers how to use a debugger while developing Orion Studio itself.

## Using Orion Studio's built-in debugger

While the Orion Studio project is open you can open the `New Process Modal` and select the `Debug` tab. There you can see two debug configurations to debug Orion Studio with, one for GDB and one for LLDB. Select the configuration you want and Orion Studio will build and launch the binary.
...
To get the full debugger experience on a release build, compile an Orion Studio binary with full debug info.
...
> If you do not want to pass `--config` on every `cargo` command, you can also change the section in the [root `Cargo.toml`](https://github.com/orion-agents/orion-studio/blob/main/Cargo.toml)
>
> from
>
> ```toml
...
## Running Orion Studio with a shell debugger GDB/LLDB

### Background

When you install Rust through rustup (the recommended setup for Orion Studio development; see your platform guide [here](../development.md)), rustup also installs helper scripts for debugging Rust binaries.
...
### Usage with Orion Studio

After enabling full debug info and building with `cargo build`, run `rust-gdb` or `rust-lldb` against the compiled Orion Studio binary:

```sh
rust-gdb target/debug/orion-studio
rust-lldb target/debug/orion-studio
```

You can also attach to a running Orion Studio process (for example, one started with `cargo run`):
...
`<pid>` is the process ID of the Orion Studio instance you want to attach to.

To find the PID, use your system's process tools, such as Task Manager on Windows or Activity Monitor on macOS.

You can also run `pgrep -fl orion-studio` on macOS and Linux, or `Get-Process | Select-Object Id, ProcessName` in PowerShell on Windows.

#### Debugging Panics and Crashes

Debuggers are useful for finding the cause of panics and crashes, including in Orion Studio.
...
You can find additional information on debugging Orion Studio crashes [here](./debugging-crashes.md).
