...
description: "Understand license requirements for publishing to Zed's upstream extension registry."
---

# Extension License Requirements {#extension-license-requirements}

> These are requirements of Zed's upstream extension registry. Orion Studio does not operate that registry or its validation service.

As of October 1st, 2025, extension repositories submitted to the upstream registry must include a license.
That registry accepts the following licenses:
...
This allows the upstream registry to distribute the resulting binary produced from your extension code.
Without a valid license, a pull request to add or update an extension in that registry will fail its CI checks.
...
Any filename that has `LICENCE` or `LICENSE` as a prefix (case insensitive) will be inspected to ensure it matches one of the accepted licenses. See the upstream [license validation source code](https://github.com/zed-industries/extensions/blob/main/src/lib/license.js).
