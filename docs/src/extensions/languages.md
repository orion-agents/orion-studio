...
description: "Overview of programming language support in Orion Studio, including built-in and extension-based languages."
---

# Language Extensions

Language support in Orion Studio has several components:
...
Each language supported by Orion Studio must be defined in a subdirectory inside the `languages` directory of your extension.
...
- `first_line_pattern` is a regular expression that can be used alongside `path_suffixes` (above) or `file_types` in settings to match files that should use this language. For example, Orion Studio uses this to identify Shell Scripts by matching [shebang lines](https://github.com/orion-agents/orion-studio/blob/main/crates/languages/src/bash/config.toml) in the first line of a script.
- `debuggers` is an array of strings that are used to identify debuggers in the language. When launching a debugger's `New Process Modal`, Orion Studio will order available debuggers by the order of entries in this array.
...
Orion Studio uses the syntax tree produced by the [Tree-sitter](https://tree-sitter.github.io) query language to implement
several features:
...
features in Orion Studio, using [JSON syntax](https://www.json.org/json-en.html) as a guiding example.
...
Orion Studio resolves them right-to-left: It first tries the rightmost capture, and if the current theme has no style for it, falls back to the next capture to the left, and so on.
...
Here Orion Studio will first try to resolve `@variable` from the theme. If the theme defines a style for `@variable`, that style is used. Otherwise, Orion Studio falls back to `@type`.
...
Orion Studio uses these to highlight matching brackets: painting each bracket pair with a different color ("rainbow brackets") and highlighting the brackets if the cursor is inside the bracket pair.
...
or a lower indentation level. If Orion Studio finds no matching block, it
leaves the indentation unchanged.

Named captures such as `@start.if` mark blocks for these rules. Unlike
`@start`, they do not change an `@indent` range.

Orion Studio checks rules in order and stops after the first matching `pattern`. Put
...
The `textobjects.scm` file defines rules for navigating by text objects. This was added in Orion Studio v0.165 and is currently used only in Vim mode.
...
For languages with closures, these typically should not count as functions in Orion Studio. This is best-effort, however, because languages like JavaScript do not syntactically differentiate between closures and top-level function declarations.
...
The `@run` capture specifies where the run button should appear in the editor. Other captures, except those prefixed with an underscore, are exposed as environment variables with a prefix of `ZED_CUSTOM_$(capture_name)` when running the code. `ZED_CUSTOM_*` is a retained task-environment compatibility namespace.
...
Orion Studio uses the [Language Server Protocol](https://microsoft.github.io/language-server-protocol/) to provide advanced language support.
...
This is useful when a language server reports custom (non-standard) semantic token types that aren't covered by Orion Studio's built-in default rules. Extension-provided rules act as sensible defaults for that language — users can always override them via `semantic_token_rules` in their settings file, and built-in default rules are only used when neither user nor extension rules match.

#### Customizing Semantic Token Styles

Orion Studio supports customizing the styles used for semantic tokens. You can define rules in your settings file, which customize how semantic tokens get mapped to styles in your theme.
...
3. **Default rules** — Orion Studio's built-in rules for standard LSP token types.
...
If your language server supports additional languages, you can use `language_ids` to map Orion Studio `languages` to the desired [LSP-specific `languageId`](https://microsoft.github.io/language-server-protocol/specifications/lsp/3.17/specification/#textDocumentItem) identifiers:
