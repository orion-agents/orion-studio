---
title: How to Migrate from RustRover to Orion Studio
description: "Guide for migrating from RustRover to Orion Studio, including settings and keybindings."
---

# How to Migrate from RustRover to Orion Studio
...
After setup, press `Cmd+Shift+O` (with JetBrains keymap) to open a folder. This becomes your workspace in Orion Studio.
...
You can also launch Orion Studio from the terminal inside any existing Cargo project with:
`orion .`
...
If you chose the JetBrains keymap during onboarding, most of your shortcuts should already feel familiar. Here's a quick reference for how Orion Studio compares to RustRover.
...
### Different Keybindings (RustRover → Orion Studio)

| Action                 | RustRover   | Orion Studio (JetBrains keymap) |
| ---------------------- | ----------- | ------------------------------- |
...
RustRover uses its own proprietary code analysis engine for Rust intelligence. Orion Studio uses rust-analyzer via the Language Server Protocol (LSP).

What this means for you:

- **Completions, go-to-definition, find usages, type inference** — All available in Orion Studio via rust-analyzer
- **Macro expansion** — Available in both (use `Cmd+Shift+M` in Orion Studio)
...
- RustRover-specific inspections (beyond Clippy) won't exist in Orion Studio
- rust-analyzer is configured via JSON in Orion Studio, not through a GUI
...
Orion Studio doesn't have a Cargo GUI. You work with Cargo through:
...
RustRover organizes auxiliary views into numbered tool windows (Project = 1, Cargo = Alt+1, Terminal = Alt+F12, etc.). Orion Studio uses a similar concept called "docks":

| RustRover Tool Window | Orion Studio Equivalent | Shortcut (JetBrains keymap) |
| --------------------- | ----------------------- | --------------------------- |
...
Note that there's no dedicated Cargo tool window in Orion Studio. Use the terminal or define tasks for your common Cargo commands.

### Debugging

Both RustRover and Orion Studio offer integrated debugging for Rust, but using different backends:

- RustRover uses its own debugger integration
- Orion Studio uses **CodeLLDB** (the same debug adapter popular in VS Code)

To debug Rust code in Orion Studio:
...
RustRover has a dedicated test runner with a visual interface showing pass/fail status for each test. Orion Studio provides test running through:
...
Several features that might require plugins in other editors are built into Orion Studio:
...
Here's what RustRover offers that Orion Studio doesn't have:
...
- **Orion Studio is open source** (primarily GPL-licensed, with Apache-licensed components)
- **Telemetry is optional** and can be disabled during onboarding or in settings

## Collaboration in Orion Studio vs. RustRover

RustRover offers Code With Me as a separate feature for collaboration. Orion Studio has collaboration built into the core experience.
...
Orion Studio has built-in AI features. If you've used JetBrains AI Assistant, here's how to get set up.
...
- [Rust in Orion Studio](../languages/rust.md) — Rust-specific setup and configuration
