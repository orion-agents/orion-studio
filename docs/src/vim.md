---
title: Vim Mode - Orion Studio
description: Full Vim emulation in Orion Studio with motions, text objects, visual mode, macros, and Orion Studio-specific extensions.
---

# Vim Mode

Orion Studio includes a Vim emulation layer. This page covers enabling and disabling vim mode, key bindings, Orion Studio-specific features, and configuration options.

## Orion Studio's vim mode design

Vim mode replicates the behavior of motions and commands where it makes sense and uses Orion Studio-specific functionality where Orion Studio's approach is better. The goal is a familiar experience that works out of the box without requiring configuration.
...
So, Orion Studio's vim mode does not replicate Vim one-to-one, but it meshes Vim's modal design with Orion Studio's modern features to provide a more fluid experience. It's also configurable, so you can add your own key bindings or override the defaults.

### Core differences

There are four types of features in vim mode that use Orion Studio's core functionality, leading to some differences in behavior:

1. **Motions**: vim mode uses Orion Studio's semantic parsing to tune the behavior of motions per language. For example, in Rust, jumping to matching bracket with `%` works with the pipe character `|`. In JavaScript, `w` considers `$` to be a word character.
2. **Visual block selections**: vim mode uses Orion Studio's multiple cursor to emulate visual block selections, making block selections a lot more flexible. For example, anything you insert after a block selection updates on every line in real-time, and you can add or remove cursors anytime.
3. **Macros**: vim mode uses Orion Studio's recording system for vim macros. So, you can capture and replay more complex actions, like autocompletion.
4. **Search and replace**: vim mode uses Orion Studio's search system, so, the syntax for regular expressions is slightly different compared to Vim. [Head to the Regex differences section](#regex-differences) for details.

> **Note:** The foundations of Orion Studio's vim mode should already cover many use cases, and we're always looking to improve it. If you find missing features that you rely on in your workflow, please [file an issue on GitHub](https://github.com/orion-agents/orion-studio/issues).

## Enabling and disabling vim mode

When you first open Orion Studio, you'll see a checkbox on the welcome screen that allows you to enable vim mode.
...
## Orion Studio-specific features

Orion Studio is built on a modern foundation that (among other things) uses Tree-sitter and language servers to understand the content of the file you're editing and supports multiple cursors out of the box.

Vim mode has several "core Orion Studio" key bindings that will help you make the most of Orion Studio's specific feature set.
...
Tree-sitter is the parser Orion Studio uses to understand the structure of your code. Orion Studio provides motions that change the current cursor position, and text objects that can be used as the target of actions.
...
These commands help you manage multiple cursors in Orion Studio.
...
The following commands help you bring up Orion Studio's completion menu, request a suggestion from GitHub Copilot, or open the inline AI assistant without leaving insert mode.
...
Orion Studio's vim mode includes features commonly provided by plugins in the Vim ecosystem:
...
Orion Studio offers two different strategies for selecting text surrounded by any quote, or any bracket. These text objects are **not enabled by default** and must be configured in your keymap to be used.
...
Vim mode allows you to open Orion Studio's command palette with `:`. You can then type to access any usual Orion Studio command. Additionally, vim mode adds aliases for popular Vim commands to ensure your muscle memory transfers to Orion Studio. For example, you can write `:w` or `:write` to save the file.
...
> **Note**: We don't emulate the full power of Vim's command line yet. In particular, commands currently do not support arguments. Please [file issues on GitHub](https://github.com/orion-agents/orion-studio) as you find things that are missing from the command palette.
...
| `:cq`           | Quit completely (close all running instances of Orion Studio) |
...
These ex commands open Orion Studio's various panels and windows.
...
This command replaces text. It emulates the substitute command in vim. The substitute command uses regular expressions, and Orion Studio uses a slightly different syntax than vim. You can learn more about Orion Studio's syntax below, [in the regex differences section](#regex-differences). Orion Studio will replace only the first occurrence of the search pattern in the current line. To replace all matches append the `g` flag.
...
Orion Studio does not ship with any command mnemonics by default, but you can define short aliases for Orion Studio commands using the `command_aliases` setting in your settings file. When you type an alias from this map in the command palette, it resolves to the mapped command.
...
- `:zlog` - Open the Orion Studio log
...
Orion Studio's key bindings are evaluated only when the `"context"` property matches your location in the editor. For example, if you add key bindings to the `"Editor"` context, they will only work when you're editing a file. If you add key bindings to the `"Workspace"` context, they will work everywhere in Orion Studio. Here's an example of a key binding that saves when you're editing a file:
...
```json [keymap]
// This key binding will work when you're editing a file. It comes built into Orion Studio by default as the workspace: save command.
{
  "context": "Workspace",
  "bindings": {
    "ctrl-s": "workspace::Save"
...
In non-modal text editors, cursor navigation typically wraps when moving past line ends. Orion Studio, however, handles this behavior exactly like Vim by default: the cursor stops at line boundaries. If you prefer your cursor to wrap between lines, override these keybindings:
...
### Restoring common text editing and Orion Studio keybindings
...
## Useful core Orion Studio settings for vim mode

Here are a few general Orion Studio settings that can help you fine-tune your Vim experience:
...
Orion Studio uses a different regular expression engine from Vim. This means that you will have to use a different syntax in some cases. Here are the most common differences:

- **Capture groups**: Vim uses `\(` and `\)` to represent capture groups, in Orion Studio these are `(` and `)`. On the flip side, in Vim, `(` and `)` represent literal parentheses, but in Orion Studio these must be escaped to `\(` and `\)`.
- **Matches**: When replacing, Vim uses the backslash character followed by a number to represent a matched capture group. For example, `\1`. Orion Studio uses the dollar sign instead. So, when in Vim you use `\0` to represent the entire match, in Orion Studio the syntax is `$0` instead. Same for numbered capture groups: `\1` in Vim is `$1` in Orion Studio.
- **Global option**: By default, in Vim, regex searches only match the first occurrence on a line, and you append `/g` at the end of your query to find all matches. In Orion Studio, regex searches are global by default.
- **Case sensitivity**: Vim uses `/i` to indicate a case-insensitive search. In Orion Studio you can either write `(?i)` at the start of the pattern or toggle case-sensitivity with the shortcut {#kb search::ToggleCaseSensitive}.

> **Note**: To help with the transition, the command palette will fix parentheses and replace groups for you when you write a Vim-style substitute command, `:%s//`. So, Orion Studio will convert `%s:/\(a\)(b)/\1/` into a search for "(a)\(b\)" and a replacement of "$1".

For the full syntax supported by Orion Studio's regex engine [see the regex crate documentation](https://docs.rs/regex/latest/regex/#syntax).
