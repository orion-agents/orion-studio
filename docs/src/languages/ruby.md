---
title: Ruby
description: "Configure Ruby language support in Orion Studio, including language servers, formatting, and debugging."
---

# Ruby

Ruby support is available through the upstream-Zed [Ruby extension](https://github.com/zed-extensions/ruby), which Orion Studio consumes through extension compatibility.
...
There are multiple language servers available for Ruby. Orion Studio supports the two following:
...
In addition to these two language servers, Orion Studio also supports:

- [rubocop](https://github.com/rubocop/rubocop) which is a static code analyzer and linter for Ruby. Under the hood, it's also used by Orion Studio as a language server, but its functionality is complementary to that of solargraph and ruby-lsp.
...
Solargraph has formatting and diagnostics disabled by default. We can tell Orion Studio to enable them by adding the following to your `settings.json`:
...
Rubocop has unsafe autocorrection disabled by default. We can tell Orion Studio to enable it by adding the following to your `settings.json`:
...
The Ruby extension provides a debug adapter for debugging Ruby code. Orion Studio's name for the adapter (in the UI and `debug.json`) is `rdbg`, and under the hood, it uses the [`debug`](https://github.com/ruby/debug) gem. The extension uses the [same activation logic](#language-server-activation) as the language servers.
