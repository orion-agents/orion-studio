---
title: Go
description: "Configure Go language support in Orion Studio, including language servers, formatting, and debugging."
---

# Go

Go support is available natively in Orion Studio.
...
Orion Studio sets the following initialization options for inlay hints:
...
to make the language server send back inlay hints when Orion Studio has them enabled in the settings.
...
Orion Studio enables the `test` code lens for `gopls` by default. This shows "run test" and "run benchmark" links above `Test` and `Benchmark` functions in `*_test.go` files. To use them, enable the `code_lens` setting:
...
You might find yourself needing to connect to an existing instance of Delve that's not necessarily running on your machine; in such case, you can use `tcp_arguments` to instrument Orion Studio's connection to Delve.
...
[
  {
    ...
    "program": "/Users/developer/Projects/language_repositories/golang/hello/hello",
    "cwd": "/Users/developer/Projects/language_repositories/golang/hello",
    "args": [],
    "env": {},
    "request": "launch",
    "mode": "exec",
...
In such case Orion Studio won't spawn a new instance of Delve, as it opts to use an existing one. The consequence of this is that _there will be no terminal_ in Orion Studio; you have to interact with the Delve instance directly, as it handles stdin/stdout of the debuggee.
