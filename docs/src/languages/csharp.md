...
C# support is available through the upstream-Zed [C# extension](https://github.com/zed-extensions/csharp), which Orion Studio consumes through extension compatibility.
...
Roslyn is enabled by default. To switch to csharp-ls or OmniSharp, add the following to your Orion Studio settings file:
...
csharp-ls can be configured in an Orion Studio settings file with:
...
OmniSharp can be configured in an Orion Studio settings file with:
