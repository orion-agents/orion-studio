---
title: TypeScript
description: "Configure TypeScript language support in Orion Studio, including language servers, formatting, and debugging."
---

# TypeScript

TypeScript and TSX support are available natively in Orion Studio.
...
By default Orion Studio uses [vtsls](https://github.com/yioneko/vtsls) for TypeScript, TSX, and JavaScript files.
...
Orion Studio sets the following initialization options to make the language server send back inlay hints (that is, when Orion Studio has inlay hints enabled in the settings).

You can override these settings in your Orion Studio `settings.json` when using `typescript-language-server`:
...
Orion Studio enables references and implementations code lenses for `vtsls` by default. These show reference counts and implementation counts above functions, classes, and interfaces. To use them, enable the `code_lens` setting:
...
Orion Studio supports debugging TypeScript code out of the box with `vscode-js-debug`.
...
- [Orion Studio Yarn documentation](./yarn.md) for a walkthrough of configuring your project to use Yarn.
- [Orion Studio Deno documentation](./deno.md)
