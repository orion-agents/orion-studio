---
title: Python
description: "Configure Python language support in Orion Studio, including language servers, formatting, and debugging."
---

# How to Set Up Python in Orion Studio

Python support is available natively in Orion Studio.

- Tree-sitter: [upstream Zed fork `zed-industries/tree-sitter-python`](https://github.com/zed-industries/tree-sitter-python)
- Language Servers:
...
You'll need both Orion Studio and Python installed before you can begin.

### Step 1: Install Python

Orion Studio does not bundle a Python runtime, so you’ll need to install one yourself.
...
## Open Your First Python Project in Orion Studio

Once Orion Studio and Python are installed, open a folder containing Python code to start working.

### Step 1: Launch Orion Studio with a Python Project

Open Orion Studio.
From the menu bar, choose File > Open Folder, or launch from the terminal:

```bash
orion path/to/your/project
```

Orion Studio will recognize `.py` files automatically using its native tree-sitter-python parser, with no plugins or manual setup required.

### Step 2: Use the Integrated Terminal (Optional)

Orion Studio includes an integrated terminal, accessible from the bottom panel. If Orion Studio detects that your project is using a [virtual environment](#virtual-environments), it will be activated automatically in newly-created terminals. You can configure this behavior with the [`detect_venv`](../reference/all-settings.md#terminal-detect_venv) setting.

## Configure Python Language Servers in Orion Studio

Orion Studio provides several Python language servers out of the box. By default, [basedpyright](https://github.com/DetachHead/basedpyright) is the primary language server, and [Ruff](https://github.com/astral-sh/ruff) is used for formatting and linting.
...
[basedpyright](https://docs.basedpyright.com/latest/) is the primary Python language server in Orion Studio beginning with Orion Studio v0.204.0. It provides core language server functionality like navigation (go to definition/find all references) and type checking. Compared to Pyright, it adds support for additional language server features (like inlay hints) and checking rules.

Note that while basedpyright in isolation defaults to the `recommended` [type-checking mode](https://docs.basedpyright.com/latest/benefits-over-pyright/better-defaults/#typecheckingmode), Orion Studio configures it to use the less-strict `standard` mode by default, which matches the behavior of Pyright. You can set the type-checking mode for your project using the `typeCheckingMode` setting in `pyrightconfig.json` or `pyproject.toml`, which will override Orion Studio's default. Read on for more details about how to configure basedpyright.

#### Basedpyright Configuration

basedpyright reads configuration options from two different kinds of sources:

- Language server settings ("workspace configuration"), which must be configured per-editor (using `settings.json` in Orion Studio's case) but apply to all projects opened in that editor
...
Language server settings for basedpyright in Orion Studio can be set in the `lsp` section of your `settings.json`.
...
For compatibility with older configurations, Orion Studio also accepts `basedpyright.analysis` as a top-level key within `settings`.
...
### How Orion Studio Uses Python Toolchains

Orion Studio uses the selected Python toolchain for your project in the following ways:
...
- If the toolchain is a virtual environment, the environment's activation script will be run automatically when you launch a new shell in Orion Studio's integrated terminal, giving you convenient access to the selected Python interpreter and dependency set.
- If a built-in language server is installed in the active virtual environment, that binary will be used instead of Orion Studio's private automatically-installed binary. This also applies to debugpy.

### Selecting a Toolchain

For most projects, Orion Studio will automatically select the right Python toolchain. In complex projects with multiple virtual environments, it might be necessary to override this selection. You can use the [toolchain selector](../toolchains.md#selecting-toolchains) to pick a toolchain from the list discovered by Orion Studio, or [specify the path to a toolchain manually](../toolchains.md#adding-toolchains-manually) if it's not on the list.

## Code Formatting & Linting

Orion Studio uses [Ruff](https://github.com/astral-sh/ruff) for formatting and linting Python code. Specifically, it runs Ruff as an LSP server using the `ruff server` subcommand.

### Configuring Formatting

Formatting in Orion Studio follows a two-phase pipeline: first, code actions on format (`code_actions_on_format`) are executed, followed by the configured formatter.
...
Like basedpyright, Ruff reads options from both Orion Studio's language server settings and configuration files (`ruff.toml`) when used in Orion Studio. Unlike basedpyright, _all_ options can be configured in either of these locations, so the choice of where to put your Ruff configuration comes down to whether you want it to be shared between projects but specific to Orion Studio (in which case you should use language server settings), or specific to one project but common to all Ruff invocations (in which case you should use `ruff.toml`).

Here's an example of using language server settings in Orion Studio's `settings.json` to disable all Ruff lints in Orion Studio (while still using Ruff as a formatter):
...
Orion Studio supports syntax highlighting for code embedded in Python strings by adding a comment with the language name.
...
Orion Studio can automatically detect debuggable Python entry points. Press F4 (or run debugger: start from the Command Palette) to see available options for your current project.
This works for:
...
Orion Studio uses `debugpy` under the hood, but no manual adapter configuration is required.
...
Issues with Python in Orion Studio typically involve virtual environments, language servers, or tooling configuration.
...
- Check your Orion Studio log (using the {#action zed::OpenLog} action) for errors related to the language server you're trying to use. This is where you're likely to find useful information if the language server failed to start up at all.
...
- Restart Orion Studio to reinitialize language server connections, or try restarting the language server using the {#action editor::RestartLanguageServer}

If the language server is failing to resolve imports, and you're using a virtual environment, make sure that the right environment is chosen in the selector. You can use the "Server Info" view to confirm which virtual environment Orion Studio is sending to the language server&mdash;look for the `* Configuration` section at the end.
