---
description: Orion Studio is a text editor that supports lots of Git features
title: Git
---

# Git

Orion Studio has built-in Git support that lets you manage version control without leaving the editor. The Git Panel shows your working tree state, staging area, and branch information. Changes you make on the command line are reflected immediately in Orion Studio.

For operations that Orion Studio doesn't support natively, you can use the integrated terminal.
...
Orion Studio monitors your repository so that changes you make on the command line are instantly reflected.
...
Orion Studio shows Git blame information on the current line. To turn this off or add a delay before it appears, go to **Version Control > Inline Git Blame**.
...
Orion Studio wraps commit messages at 72 characters (a Git convention). To change this, search for "Git Commit" in Settings and adjust **Preferred Line Length**.

## Project Diff

You can see all of the changes captured by Git in Orion Studio by opening the Project Diff ({#kb git::Diff}), accessible via the {#action git::Diff} action in the Command Palette or the Git Panel.
...
By default, Orion Studio highlights changed words within modified lines to make it easier to spot exactly what changed. To disable this globally, open the Settings Editor and go to **Languages & Tools > Miscellaneous**, then turn off **Word Diff Enabled**.
...
Orion Studio displays diffs in two modes: **split** (side-by-side comparison) or **unified** (inline changes). Split view is the default.
...
See [Configuring Orion Studio](./configuring-zed.md) for more about the Settings Editor.
...
Fetch, push, or pull from your Git repository in Orion Studio via the buttons available on the Git Panel or via the Command Palette by looking at the respective actions: {#action git::Fetch}, {#action git::Push}, and {#action git::Pull}.

### Push Configuration

Orion Studio respects Git's push configuration. When pushing, Orion Studio checks the following in order:

1. `pushRemote` configured for the current branch
2. `remote.pushDefault` in your Git config
3. The branch's tracking remote

This matches Git's standard behavior, so if you've configured `pushRemote` or `pushDefault` in your `.gitconfig` or via `git config`, Orion Studio will use those settings.

## Remotes

When your repository has multiple remotes, Orion Studio shows a remote selector in the Git Panel. Click the remote button next to push/pull to choose which remote to use for that operation.

## Staging Workflow

Orion Studio has two primary staging workflows, using either the Project Diff or the panel directly.
...
Orion Studio offers two commit textareas:
...
As soon as you commit in Orion Studio, in the Git Panel, you'll see a bar right under the commit textarea, which will show the recently submitted commit.
In there, you can use the "Uncommit" button, which performs the `git reset HEADˆ--soft` command.

### Configuring Commit Line Length

By default, Orion Studio sets the commit line length to `72` but it can be configured in your local `settings.json` file.
...
To delete a branch, open the branch switcher with {#action git::Switch}, find the branch you want to delete, and use the delete option. Orion Studio will confirm before deleting to prevent accidental data loss.
...
- Type a name to create a named worktree or let Orion Studio automatically pick one for you
...
This keeps Orion Studio from accidentally checking out the same branch in multiple worktrees.

The directory used for new worktrees is controlled by the `git.worktree_directory` setting.
By default, Orion Studio creates worktrees under `../worktrees` relative to the repository's working directory.

See [All Settings](./reference/all-settings.md#git-worktree-directory) for examples.

### Init Setup

To run setup steps after Orion Studio creates a linked worktree, use the [`create_worktree` task hook](./tasks.md#hooks).
For agent-specific workflows, see [Worktree Isolation](./ai/parallel-agents.md#worktree-isolation).

### Multi-root Workspaces

If your project contains multiple Git repositories (i.e., multi-root folders), Orion Studio creates a linked worktree for each repository when creating a new worktree from the picker.
Non-Git folders in the same project are included in the new workspace as-is.

## Merge Conflicts

When you encounter merge conflicts after a merge, rebase, or pull, Orion Studio highlights the conflicting regions in your files and displays resolution buttons above each conflict.
...
Orion Studio provides a stash picker accessible via {#action git::ViewStash} or from the Git Panel's overflow menu. From the stash picker, you can:
...
For faster workflows, Orion Studio provides direct actions to work with the most recent stash:
...
Orion Studio currently supports LLM-powered commit message generation.
...
> Note that you need to have an LLM provider configured either via your own API keys or through Orion Studio's hosted AI models.
...
To add custom commit instructions for the model, use the global `AGENTS.md` file located at `~/.config/orion-studio/AGENTS.md` on macOS and Linux, `%APPDATA%\Orion Studio\AGENTS.md` on Windows.
...
These instructions are sent to the model in addition to any instruction files, such as `.rules` or `AGENTS.md`. To add instructions that apply to both commit messages and the agent more broadly, use the global `AGENTS.md` file located at `~/.config/orion-studio/AGENTS.md` on macOS and Linux, `%APPDATA%\Orion Studio\AGENTS.md` on Windows.

> Before Orion Studio v1.4.0, this was done through the Rules Library, which has been removed.
> See [Migrating from Rules](./ai/instructions.md#migrating-from-rules) for more information.

## Git Integrations

Orion Studio integrates with popular Git hosting services to ensure that Git commit hashes and references to Issues, Pull Requests, and Merge Requests become clickable links.

Orion Studio currently supports links to the hosted versions of
...
Orion Studio automatically identifies Git hosting providers by checking for keywords in your Git remote URL. For example, if your self-hosted URL contains `gitlab`, `gitea`, or other recognized provider names, Orion Studio will automatically register that hosting provider without any configuration needed.

However, if your self-hosted Git instance URL doesn't contain identifying keywords, you can manually configure Orion Studio to create clickable links to your instance by adding a `git_hosting_providers` setting so commit hashes and permalinks resolve to your domain:
...
Orion Studio also has a Copy Permalink feature to create a permanent link to a code snippet on your Git hosting service.
...
When viewing files with changes, Orion Studio displays diff hunks that can be expanded or collapsed for detailed review:
...
To use Orion Studio as your [Git commit-message editor](https://git-scm.com/book/en/v2/Customizing-Git-Git-Configuration#_core_editor), configure the preferred `orion` launcher with `--wait`:

```sh
git config --global core.editor "orion --wait"
```

Or add the following to your shell environment (in `~/.zshrc`, `~/.bashrc`, etc):

```sh
export GIT_EDITOR="orion --wait"
```
