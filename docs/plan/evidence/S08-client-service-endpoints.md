...
- **Out of scope (→ S11):** user-facing `zed.dev` doc/help/status/merch/schema URLs in other crates are _not_ service endpoints and are deferred to a follow-up brand-URL sweep (no `orion.dev` docs/status/merch site exists yet).
