# Orion Studio Documentation Conventions

This document covers structural conventions for Orion Studio documentation: what to document, how to organize it, and when to create new pages.
...
- **Version-specific behavior changes** — Include verified version callouts (e.g., "In Orion Studio [VERSION] and above...")
...
- Users would search for it **by name** (e.g., "Orion Studio terminal", "Orion Studio snippets")
...
```yaml
---
title: Feature Name - Orion Studio
description: One sentence describing what this page covers. Used in search results.
---
```

- `title`: Feature name, optionally with "- Orion Studio" suffix for SEO
...
- Commands: `:w`, `:q`
- File paths: `~/.config/orion-studio/settings.json`
- Action names: `git::Commit`
- Values: `true`, `false`, `"eager"`

### Action and Keybinding References

Use Orion Studio's special syntax for dynamic rendering:
...
```markdown
> **Note:** In Orion Studio [VERSION] and above, tool approval is controlled by `agent.tool_permissions.default`.
```
...
- Link to `orion.dev` pages when appropriate
...
- For main feature pages with a matching marketing page, include a relevant
  `orion.dev` marketing link in addition to docs links
...
Keep language docs focused on Orion Studio-specific configuration, not general language tutorials.
...
- **macOS/Linux:** `~/.config/orion-studio/settings.json`
...
- [ ] Main feature pages include a relevant `orion.dev` marketing link
