...
Prefer the upstream `zed-bench` Instruments template, which combines Time Profiler data, CPU counters, and hang or microhang tables. Fall back to `Time Profiler` if it is unavailable:

```sh
rm -rf /tmp/orion-studio-profiles/<name>.trace
xctrace record \
  --template "zed-bench" \
  --output /tmp/orion-studio-profiles/<name>.trace \
