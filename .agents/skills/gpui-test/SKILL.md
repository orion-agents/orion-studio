...
name: gpui-test
description: >-
  Use when writing, debugging, or reproducing GPUI tests in Orion Studio, including
...
- `ZED_HEADLESS=1` — forces GPUI platform guessing toward headless mode; useful for tests that otherwise interact with platform/window setup. This inherited test/ABI variable remains read-only compatibility input until the runtime exposes a canonical Orion replacement.
