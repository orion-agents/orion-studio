...
const NEEDS_REVIEW_PULLS_URL: &str = "https://github.com/orion-agents/orion-studio/pulls?q=is%3Apr+is%3Aclosed+label%3A%22PR+state%3Aneeds+review%22";
...
fn validate_release_assets(deps: &[&NamedJob]) -> NamedJob {
    ...
    let validation_script = formatdoc! {r#"
        ...
        ACTUAL_ASSETS=$(gh release view "$TAG" --repo=orion-agents/orion-studio --json assets -q '[.assets[].name]')
    ...
    fn auto_release_preview(token: &StepOutput) -> Step<Run> {
        named::bash(indoc::indoc! {r#"
            ...
            if [[ "$should_release" == "true" ]]; then
                gh release edit "$tag" --repo=orion-agents/orion-studio --draft=false
...
fn upload_release_assets(deps: &[&NamedJob], bundle: &ReleaseBundleJobs) -> NamedJob {
    ...
    named::job(
        dependant_job(&deps)
            ...
            .add_step(
                steps::script("gh release upload \"$GITHUB_REF_NAME\" --repo=orion-agents/orion-studio release-artifacts/*")
